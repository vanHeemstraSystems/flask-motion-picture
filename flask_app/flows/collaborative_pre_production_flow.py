...
@task
def casting(
    casting_director: CastingDirector, principal_cast: PrincipalCast, actors: list
):
    for actor in actors:
        print(
            f"{casting_director.name} is casting {actor.name} for the role of {actor.role}."
        )
...
def location_scouting(director: Director, producer: Producer, locations: list):
    for location in locations:
        print(
            f"{director.name} and {producer.name} are scouting {location.name}: {location.description}."
        )
...
def arrange_art(art_personal_assistant: ArtPersonalAssistant, scenes: list):
    for scene in scenes:
        print(
            f"{art_personal_assistant.name} is arranging art elements for scene: {scene.description}."
        )


@flow
def collaborative_pre_production_flow(
    director: Director,
    producer: Producer,
