...
def create_cgi_branch(animator: Animator, branch_name: str):
    ...
    return CGIAnimationBranch(branch_name=branch_name, content="Initial CGI content", animator_id=animator.id)
...
def submit_cgi_pull_request(branch: CGIAnimationBranch, reviewer: Animator):
    print(f"Pull request submitted for CGI branch {branch.branch_name} to {reviewer.name}.")
    return CGIPullRequest(branch_id=branch.id, reviewer_id=reviewer.id, status="Pending")
...
@flow
def iterative_cgi_flow(animator: Animator, reviewer: Animator, branch_name: str, new_content: str):
