...
def create_sound_design_branch(sound_designer: SoundDesigner, branch_name: str):
    ...
    return SoundDesignBranch(
        branch_name=branch_name,
        content="Initial sound design content",
        sound_designer_id=sound_designer.id,
    )
...
@task
def submit_sound_design_pull_request(
    branch: SoundDesignBranch, reviewer: SoundDesigner
):
    print(
        f"Pull request submitted for sound design branch {branch.branch_name} to {reviewer.name}."
    )
    return SoundDesignPullRequest(
...
def merge_sound_design_to_main(branch: SoundDesignBranch):
    print(
        f"Sound design branch {branch.branch_name} has been merged into the main branch."
    )


@flow
def iterative_sound_design_flow(
    sound_designer: SoundDesigner,
    reviewer: SoundDesigner,
    branch_name: str,
    new_content: str,
):
